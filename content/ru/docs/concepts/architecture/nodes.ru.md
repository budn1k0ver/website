...
Раздел `Conditions` описывает состояния всех `Running` узлов. Примеры состояний:

| Состояние узла |   Описание  |
|----------------|-------------|
| `Ready`        | `True` если узел успешно запущен и готов к запуску подов, `False` если при запуске узла возникли проблемы, и `Unknown` если контроллер узла не получил ответа от узла в течении `node-monitor-grace-period` (по умолчанию 40 секунд) |
| `MemoryPressure`    | `True` если память узла мала, иначе `False` |
| `PIDPressure`    | `True` если на узле слишком много процессов, иначе `False` |
| `DiskPressure`    | `True` если на диске мало памяти, иначе `False` |
| `NetworkUnavailable`    | `True` если настройки сети для узла не корректны, т.е. не верно настроены, иначе `False` |

Состояние успешно запущеного узла в ввиде JSON объекта:
...
Если значение состояния `Ready` остаётся `Unkown` или `False` на протяжении более чем указано в `pod-eviction-timeout` (параметр доступен для настройки в [kube-controller-manager](/docs/admin/kube-controller-manager/)), тогда все поды узла буду запланированы на удаление с помощью [Node Controller](#node_controller). По умолчанию значение параметра `pod-eviction-timeout` равно 5 минутам. Возможна ситуация, когда узел становится не достижим и apiserver не может связаться с kubelet узла. Принятое решение об удалении подов не может быть передано в kubelet пока связь с apiserver не будет востановлена.
В тоже время поды, запланированные на удаление, могут продолжать работать на узле.

В версиях Kubernetes до 1.5, [Node Controller](#node_controller) мог [принудительно удалить](/docs/concepts/workloads/pods/pod/#force-deletion-of-pods) недоступные для apiserver поды. Однако, в версии 1.5 и выше, [Node Controller](#node_controller) перестал это делать, пока не подтвердится что они больше не работают в кластере.
Вы можете видеть поды запущенные на недоступном узле, как находящиеся в состоянии `Terminating` или `Unknown`. В случаях когда Kubernetes не может сделать вывод, был ли узел удален из кластера или нет, то администратору этого кластера может потребоваться удалить объект узла в ручную. Удаление узла из Kubernetes приводит к удалению всех подов, работающих на этом узле, и высвобождению их имен.

Контроллер жизненого цикла узла автоматически создает [недостатки](/docs/concepts/configuration/taint-and-toleration/) (taints), которые представляю собой состояния. Планировщик принимает во внимание недостатки узла при назначении ему пода. Поды также могут иметь допущения (tolerations), которые позволяют им соглашаться с недостатками узла.
